...
void customBoardAkel(std::map<std::pair<char, char>, char> & gameBoard)
{
    std::map<std::pair<char, char>, char> temp;
    for (char y = '1'; y <= '8'; y++) {
        for (char x = 'a'; x <= 'h'; x++)
...
//Prints board into console output
...
void checkPromote(std::map<std::pair<char, char>, char> & gameBoard) {
    for (auto el : gameBoard) {
        ...
        if ((el.first).second == '1')
            if (el.second == pieces[Red])
                gameBoard[el.first] = pieces[RedKing];
    }
}
...
std::pair< std::vector<std::pair<char, char>>, std::vector<std::pair<char, char>>> findJumpSquares(const char & playerPiece,
    ...
    if(playerPiece != pieces[Red]){ //pieces that can go forward
...
bool singleSquareMove(const std::pair<char, char> & from,
    ...
    int amtSquares = 0;
    if (playerPiece == pieces[Black] || playerPiece == pieces[BlackKing] || playerPiece == pieces[RedKing]) { //Pieces that can move up
        //top right square
        if(findAllSquares) {
    ...
    if (playerPiece == pieces[Red] || playerPiece == pieces[BlackKing] || playerPiece == pieces[RedKing]) { // Pieces that can move up
        //bottom left square
        if(findAllSquares) {
            auto it = gameBoard.find(std::make_pair(from.first - 1, from.second - 1));
            if(it != gameBoard.end()){
...
int checkWinStatus(std::map<std::pair<char, char>, char> & gameBoard, int & playerTurn){
    if ((findPiecesRemaining(Red, gameBoard)).size() == 0) {
        ...
        if(checkStalemate( ((playerTurn == Black) ? Red : Black), gameBoard ))
            return Draw; //Both are at stalemate
        //std::cout << "Stalemate: [" << pieces[playerTurn] << "] loses." << std::endl;
        return ((playerTurn == Black) ? RedWin : BlackWin);
    }
    else{
        return ValidMove;
    }
